...
class VGG(nn.Module):
    def __init__(self, features, num_classes=10, init_weights=True, pooling="avg"):
    ...
    def forward(self, x):
        ...
        for layer in self.classifier:
            if isinstance(layer, nn.ReLU):
                ...
                # CrypTen has a bug with dropout. It is not needed during inference anyways.
                if self.training:
                    x = layer(x)
            else:
                x = layer(x)
...
def make_layers(cfg, batch_norm=False, pooling="avg"):
    layers = []
    in_channels = 3
    for v in cfg:
        if v == "M":
...
def _vgg(arch, cfg, batch_norm, pretrained, progress, device, **kwargs):
    ...
    model = VGG(make_layers(cfgs[cfg], batch_norm=batch_norm, pooling=kwargs["pooling"]), **kwargs)
    if pretrained:
